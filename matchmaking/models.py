...
class Tree(Models):
    ...
    def calculate_tree_distance(cls, samples, dataframe_features, dataframe_bool):
        ...
        for sample in samples:
            ...
            features = tmp_features[tmp_features[cls.model_id].eq(sample)]['feature'].tolist()
            if not features:
                empty_counter += 1
            ...
            relative_booleans = (relative_booleans
                                 ...
                                 .reset_index()
                                 .reset_index()
                                 .rename(columns={cls.model_id: cls.comparison, 'index': cls.label})
                                 )
    ...
    def create_boolean_table(cls, dataframe, samples, features):
        dataframe.set_index(['feature', cls.model_id], inplace=True)
        dataframe['value'] = 1
        ...
        return (df
                .reset_index().rename(columns={'level_0': 'feature', 'level_1': cls.model_id})
                .pivot_table(index=cls.model_id, columns='feature', values='value')
                )
    ...
    def create_sorted_table(cls, inputs, samples):
        ...
        df = pd.concat([
            variants.loc[:, [cls.model_id, 'feature'] + sort_columns],  # + ' mut'
            copy_number_alterations.loc[:, [cls.model_id, 'feature'] + sort_columns],  # + '  CN'
            fusions_gene1.loc[:, [cls.model_id, 'feature'] + sort_columns],  # + '  Fusion'
            fusions_gene2.loc[:, [cls.model_id, 'feature'] + sort_columns]  # + '  Fusion'
        ])

        df = df.sort_values(sort_columns, ascending=False).drop_duplicates(subset=[cls.model_id, 'feature'], keep='first')
        return df[df[cls.model_id].isin(samples)].reset_index(drop=True)
