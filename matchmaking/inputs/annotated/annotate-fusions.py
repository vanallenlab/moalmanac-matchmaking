...
def annotate_both_genes(
    df_gene1, df_gene2, db_gene1, db_gene2, almanac_genes, df, dbs, config
):
    # Gene1 and Gene 2
    ...
    df = df.sort_values(["evidence", "feature_match"], ascending=False).drop_duplicates(
        ["feature", "partner", "sample_name"], keep="first"
    )
...
def annotate_gene_1(df_gene1, db_gene1, db_gene2, almanac_genes, dbs, config):
    # Gene 1
    ...
    ).drop_duplicates(["feature", "sample_name"], keep="first")
...
def annotate_gene_2(df_gene2, db_gene1, db_gene2, almanac_genes, dbs, config):
    # Gene 2
...
def load_db(almanac_json, table_label):
    db = pd.DataFrame(almanac_json["content"])
...
def load_df(handle, feature_type):
    df = pd.read_csv(handle, sep="\t", usecols=["sample_name", "feature", "partner"])
...
if __name__ == "__main__":
    ...
    almanac_genes = almanac_json['genes']
    ...
    both_genes = annotate_both_genes(
        df_gene1, df_gene2, db_gene1, db_gene2, almanac_genes, dataframe, dbs, config
    )
    gene1 = annotate_gene_1(df_gene1, db_gene1, db_gene2, almanac_genes, dbs, config)
    gene2 = annotate_gene_2(df_gene2, db_gene1, db_gene2, almanac_genes, dbs, config)
